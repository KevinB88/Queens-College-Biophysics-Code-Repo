...
def u_tube_ii(rho, phi, k, m, n, a, b, v, d_time, d_radius, d_theta):
    j_l = v * rho[k][m]
    if m == len(phi[k][m]) - 1:
        j_r = 0
    else:
...
# global_diffusive_mass = np.zeros([2565])
# global_advective_mass = np.zeros([2565])
...
def calc_mass(phi, rho, k, d_radius, d_theta, curr_central, rings, rays):
    ...
    #
    # global_advective_mass[k] = microtubule_mass
            ...
            for n in range(rays):
                if m == rings - 1:
                    ...
                    if n == angle:
                        ...
                        # def u_tube_iii(rho, phi, k, m, n, a, b, v, d_time, d_radius):
                        rho_tab[k+1][m] = u_tube_ii(rho_tab, phi_tab, k, m, n, a, b, v, d_time, d_radius, d_theta)
...
# def domain_masses():
#     phi = global_diffusive_mass
#     rho = global_advective_mass
#
#     return phi, rho
