...
def solve(rings, rays, r, d, t, q, p, a, b, v, tube_placements, output_specification):
            ...
            if m == rings - 2:
                ...
                if k > 0 and global_max_flag:
                    if mass_loss_tab[k-1] > mass_loss_tab[k]:
                        ...
                        # appended_array = np.concatenate((original_array, zeros_to_append), axis=2)
            for n in range(rays):
                if m == rings - 1:
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    output_dictionary = {
        ...
        'total_mass': mass_tab,
        'mass_loss_rate_j_r_r': mass_loss_tab,
    ...
    return np.array([output_dictionary[key] for key in output_dictionary if key in [token.lower() for token in output_specification]]), d_time, d_radius, diffusive_layer
