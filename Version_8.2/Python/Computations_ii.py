...
def u_density(phi, k, m, n, d_radius, d_theta, d_time, central, rings, rho_dict, dict_index, a, b):
    ...
    angles_list = list(rho_dict.keys())

    if n == angles_list[dict_index]:  # if the current angle 'n' equates to the angle for which the microtubule is positioned at
        component_c = (a * phi[k][m][n]) * d_time - (((b * rho_dict[angles_list[dict_index]][k][m]) * d_time) / ((m+1) * d_radius * d_theta))
        # if m == 0:
...
def u_center(phi, k, d_radius, d_theta, d_time, curr_central, rho_dict, v):
    total_sum = 0
    ...
    for i in range(len(rho_dict)):
        j_l = rho_dict[angles_list[i]][k][0] * v
...
def calc_mass(phi, rho_dict, k, d_radius, d_theta, curr_central, rings, rays):
    mass = 0
    ...
    angles_list = list(rho_dict.keys())
    for i in range(len(rho_dict)):
        for m in range(len(rho_dict[angles_list[i]][k])):
            microtubule_mass += rho_dict[angles_list[i]][k][m] * d_radius
...
def solve(rings, rays, r, d, t, q, p, a, b, v, tube_placements):
        ...
        for m in range(rings):
            ...
            for n in range(rays):
                if m == rings - 1:
                    ...
                    phi_tab[k + 1][m][n] = u_density(phi_tab, k, m, n, d_radius, d_theta, d_time, phi_center, rings, rho_dict, dict_index, a, b)

                    angles_list = list(rho_dict.keys())

                    if n == angles_list[dict_index]:
                        # Update the associated tube within the dictionary
                        rho_dict[angles_list[dict_index]][k+1][m] = u_tube(rho_dict[angles_list[dict_index]], phi_tab, k, m, n, a, b, v, d_time, d_radius, d_theta)
                        if dict_index < len(angles_list)-1:
                            dict_index = dict_index + 1

        central_phi_tab[k] = phi_center

        mass_tab[k] = calc_mass(phi_tab, rho_dict, k, d_radius, d_theta, phi_center, rings, rays)

        # QUESTION: How should the center be updated in this case with the new implementation of rho_dict?
        phi_center = u_center(phi_tab, k, d_radius, d_theta, d_time, phi_center, rho_dict, v)
    ...
    return phi_tab, central_phi_tab, mass_tab, mass_loss_tab, mass_loss_tab_ii, d_time, d_radius, rho_dict, info_dict
