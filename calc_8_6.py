...
def u_density(phi, rho, k, m, n, d_radius, d_theta, d_time, central, rings, angle, a, b):
    ...
    if n == angle:  # if the current angle 'm' equates to the angle for which the microtubule is positioned at
...
def u_tube(rho, phi, k, m, n, a, b, v, rings, d_time, d_radius):
    ...
    return rho[k][m] - diff + (a * phi[k][m][n] * d_time) - (b * rho[k][m] * d_time)
...
def solve(rings, rays, r, d, t, q, p, a, b, v, angle):
    ...
    return phi_tab, central_phi_tab, mass_tab, mass_loss_tab, mass_loss_tab_ii, d_time, d_radius, rho_tab
