...
def plot_multiple_general(container, key, save_png, filepath, show_plot, x_lbl, y_lbl):
    ...
    for i in range(len(container)):
        ...
        print(f'{key[i]}')
        plt.plot(df[f'{x_lbl}'], df[f'{y_lbl}'], label=f'{key[i]}')
...
def plot_phi_angle(container, ring, time, discrete, save_png, filepath, plot_show):
...
def plot_rho_density(container, radius, time, text, filepath, save_png, discrete, plot_show):
...
def plot_mass_loss(container, time, save_png, filepath, plot_show):
...
def plot_rho_versus_radius(rho_container, radius):
    x = np.linspace(0, radius, len(rho_container[len(rho_container)-1]))
    y = rho_container[len(rho_container)-1]
...
def plot_mass(mass_container, time, save_png, filepath, plot_show):
...
def plot_central_density(central_container, time, save_png, filepath, plot_show):
...
def plot_final_density(container, radius, angle, save_png, filepath, time, plot_show):
    ...
    plt.title(f'Phi versus radius at angle: {angle}')
    plt.plot(x, y, 'black')
    plt.xlabel('Radius')
    plt.ylabel('Density')
