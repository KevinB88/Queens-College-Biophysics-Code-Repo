...
def tabulate_density_angles(container, filepath, time, ring):
...
def tabulate_rho_density(container, filepath, time, delta_radius):
    ...
    current_time = datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"rho_density_data_{current_time}_{time}.csv"
...
def tabulate_mass_loss(container, filepath, time, delta_time, w):
    ...
    current_time = datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"mass_loss_data_{current_time}_{time}_w{w}.csv"
...
def tabulate_densities(container, filepath, angle, time, delta_radius):
    final_state = container[len(container)-1]
    ...
    current_time = datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"density_across_radius_data_date{current_time}_time{time}_angle{angle}.csv"
