...
def solve(rings, rays, r, d, t, q, p, a, b, v, tube_placements, output_specification, print_info_dictionary):
    ...
    for k in range(time_steps):
        ...
        for m in range(rings):
            ...
            if m == rings - 2:
                ...
                if k > 0 and output_specification == 'global-max':
                    if mass_loss_tab[k-1] > mass_loss_tab[k]:
                        # glb_max_mass_loss = mass_loss_tab[k-1]
                        glb_peak_time = (k-1) * d_time
                        return glb_peak_time
                        # return mass_loss_tab, mass_loss_tab_ii, glb_peak_time, glb_max_mass_loss
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    if output_specification.lower() == 'mean-flow-passage-time':
        if print_info_dictionary:
            return m_f_p_t, info_dict
        return m_f_p_t
    elif output_specification.lower() == 'diffusive_layer':
        if print_info_dictionary:
            return diffusive_layer, info_dict
        return diffusive_layer
    elif output_specification.lower() == 'central_patch':
        if print_info_dictionary:
            return central_phi_tab, info_dict
        return central_phi_tab
    elif output_specification.lower() == 'advective_layer':
        if print_info_dictionary:
            return advective_layer, info_dict
        return advective_layer
    elif output_specification.lower() == 'total_mass':
        if print_info_dictionary:
            return mass_tab, info_dict
        return mass_tab
    elif output_specification.lower() == 'mass_loss_rate_j_r_r':
        if print_info_dictionary:
            return mass_loss_tab, info_dict
        return mass_loss_tab
    elif output_specification.lower() == 'mass_loss_rate_derivative':
        if print_info_dictionary:
            return mass_loss_tab_ii, info_dict
        return mass_loss_tab_ii
